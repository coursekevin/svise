...
def get_experiment(system_name):
    ...
    # want both alpha and beta to have zero mean
    alpha_bnds, beta_bnds = corruption_tuning[system_name]
    alpha = npr.rand() * alpha_bnds - alpha_bnds / 2
    beta = npr.rand() * beta_bnds - beta_bnds / 2
    # print(f"alpha: {alpha}, beta: {beta}")
    def corrupted_ode(t, x):
        return corruption(t, x, odes.torch_ode(t, x, test_ode), alpha, beta)

    return num_states, test_ode, corrupted_ode, tf, x0, noise, W, alpha, beta
...
for key in tqdm(system_list):
    ...
    for i in range(num_indep_datasets):
        ...
        d, test_ode, corrupted_ode, tf, x0, _, W, alpha, beta = get_experiment(key)
